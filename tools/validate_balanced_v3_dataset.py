"""Validate Balanced V3 dataset properties and report detector-relevant distribution stats."""
...
def main() -> int:
    ...
    parser.add_argument("--dataset", type=Path, default=ROOT / "yolo_dataset_balanced_v3")
    ...
    if stats.get("split_mode") != "balanced-v3":
    ...
    for item in manifest:
        ...
        transform_counts[split][item.get("transform", "")] += 1
        if split in {"val", "test"} and not is_base_transform(item.get("transform", "")):
    ...
    for source in sorted(REQUIRED_SOURCES & set(by_source_split)):
        for split in ("train", "val", "test"):
            if by_source_split[source][split] <= 0:
                errors.append(f"{source} has no {split} samples")
    ...
    if errors:
        print("balanced-v3 validation failed")
        for error in errors:
            print(f"- {error}")
        return 1

    print("balanced-v3 validation ok")
    print(f"v3_profile={stats.get('augmentation', {}).get('v3_profile', '')}")
    for split in ("train", "val", "test"):
